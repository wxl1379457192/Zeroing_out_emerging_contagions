function [sim] = ISEIRV_NPI_model(data, NPI, Controls, par, R0, tspan, latent)
...
if isempty(I05(I05>=1))
    ...
    sim(1,5) = array2table(max(I05(I05>=1))-min(I05(I05>=1)));
    sim(1,8) = array2table(sum(I05(I05>=1)));
    sim(1,11) = array2table(max(I05(I05>=1)));
end

if isempty(Inew(Inew>=1))
    sim(1,1) = array2table(0);
    sim(1,4) = array2table(0);
    sim(1,7) = array2table(0);
    sim(1,10) = array2table(0);
else
    sim(1,1) = array2table(mean(Inew(Inew>=1)));
    sim(1,4) = array2table(max(Inew(Inew>=1))-min(Inew(Inew>=1)));
    sim(1,7) = array2table(sum(Inew(Inew>=1)));
    sim(1,10) = array2table(max(Inew(Inew>=1)));
end

if isempty(I95(I95>=1))
    sim(1,3) = array2table(0);
    sim(1,6) = array2table(0);
    sim(1,9) = array2table(0);
    sim(1,12) = array2table(0);
else
    sim(1,3) = array2table(mean(I95(I95>=1)));
    sim(1,6) = array2table(max(I95(I95>=1))-min(I95(I95>=1)));
