...
dataset = readtable('E:/zeroCOVID_NPI/Rt_0709/codeV0803/1025dataset/Rt&smooth_NPI.csv',VariableNamingRule='preserve');
NPIefficacy = readtable('E:/zeroCOVID_NPI/Rt_0709/codeV0803/SEIR_1214/NPIefficacy.csv',VariableNamingRule='preserve');
%dataset = testdata;
%load('NPIseffect.mat')
%load('dataset.mat')
%%
%dataset = RtsmoothNPI;
outdir = 'E:/zeroCOVID_NPI/Rt_0709/codeV0803/SEIR_1220';
...
for i = 1 : size(index,1)
    ...
    switch string(data.VG(1))
        case 'original&alpha'
            [Effect_alpha{i,1},Effect_alpha{i,2},val{i,7},val{i,8},val{i,1},val{i,2},val{i,3}] = ISEIRV_validation_model(data, NPIs, i, outdir);
            k = mean(Effect_alpha{i,1});
            for j = 1:8
                Par(i,j) = {k(j)};
            end
            val(i,4) = {string(data.VG(1))};
        ...
        case 'delta'
            [Effect_delta{i,1},Effect_delta{i,2},val{i,7},val{i,8},val{i,1},val{i,2},val{i,3}] = ISEIRV_validation_model(data, NPIs, i, outdir);
            ...
            Par(i,11) ={convertTo(data.original_start(1),'yyyymmdd')};
        case 'omicron'
            [Effect_omicron{i,1},Effect_omicron{i,2},val{i,7},val{i,8},val{i,1},val{i,2},val{i,3}] = ISEIRV_validation_model(data, NPIs, i, outdir);
