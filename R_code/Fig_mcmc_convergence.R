...
path = "Reviewed_IndividualNPI_stanV5_pooled_onebeta"
outpath = "Figures_Reviewed"
efdata<-list()
list<-list.files(path,pattern="*.rds")
...
valplot[[1]]<-ggplot(conresult,aes(rhat))+
  geom_histogram(bins=200,colour="white",fill="#2D6587",size=0.01)+
...
ggsave(paste0(outpath,"/mcmc_convergence.jpg"),plot,units="mm",width=150,height=40)
############################################################################################
