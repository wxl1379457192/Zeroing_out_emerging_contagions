...
dir = "SEIR_simulation_P4_facial0.75/"
indir = "dataset"
...
my_plot_fun <- function(data){
  ...
  data$value[which(data$variable=="Start.NPI")] = data$value[which(data$variable=="Start.NPI")]/15
...
annotation_fun1 <- function(data, infection_pre,Duration,pop, plot_fun) {
  ...
  sub_grob <- annotation_custom(ggplotGrob(subplot), 
                                ...
                                y = Duration-sqrt(as.numeric(pop))-3, 
                                xmax =  sqrt(infection_pre+sqrt(as.numeric(pop))*1),
                                ymax = Duration+sqrt(as.numeric(pop))+3)
}
...
annotation_fun3 <- function(data, infection_pre,Duration,pop, plot_fun) {
  subplot = plot_fun(data)
  sub_grob <- annotation_custom(ggplotGrob(subplot), 
                                x = sqrt(infection_pre-sqrt(as.numeric(pop))*5), 
                                y = Duration-sqrt(as.numeric(pop))-3, 
                                xmax =  sqrt(infection_pre+sqrt(as.numeric(pop))*5),
                                ymax = Duration+sqrt(as.numeric(pop))+3)
}
library(maps)
library(tidyverse)
...
for(r in split(df,df$R0)){
  ...
  for(l in split(r,r$Latent)){
    ...
    g = g + scale_x_sqrt(limits = c(round(min(l1$infection_pre),0)-10,round(max(l1$infection_pre),0)+10))
    ...
    ggsave(paste0(outdir,"/Fig4_R0",unique(l$R0),"_Latent",unique(l$Latent),"_facial0.75.pdf"),g, width=7, height=6, units="cm", scale=1)
  }
}

write.csv(A,paste0(outdir,"/Fig4_data_facial0.75.csv"),row.names = F)
