...
dir = "SEIR_simulation_P4_facial0.25/"
indir = "dataset"
...
for(r in split(df,df$R0)){
  ...
  for(l in split(r,r$Latent)){
    ...
    ggsave(paste0(outdir,"/Fig4_R0",unique(l$R0),"_Latent",unique(l$Latent),"_facial0.25.pdf"),g, width=7, height=6, units="cm", scale=1)
  }
}

write.csv(A,paste0(outdir,"/Fig4_data_facial0.25.csv"),row.names = F)
