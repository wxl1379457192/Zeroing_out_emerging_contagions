...
alldir = "Reviewed_Bayesian_model_validation"
...
valdir = "Reviewed_IndividualNPI_stanV5_pooled_onebeta"
...
par = do.call(rbind,lapply(conlist,function(c){
  ...
  if(unique(data$VG)=="original&alpha"){
    r0 = mean(out$R0_alpha)
    alv = al[,1:length(NPIname)] 
    colnames(alv)=NPIname
    for (i in NPIname){
      data[,i] =  -quantile(alv[,i])[3]*data[,i]
    }
  }else if(unique(data$VG)=="delta"){
    r0 = mean(out$R0_delta)
    alv = al[,(length(NPIname)+1):(2*length(NPIname))] 
    colnames(alv)=NPIname
    for (i in NPIname){
      data[,i] =  -quantile(alv[,i])[3]*data[,i]
    }
  }else{
    r0 = mean(out$R0_omicron)
    alv = al[,(2*length(NPIname)+1):(3*length(NPIname))] 
    colnames(alv)=NPIname
    for (i in NPIname){
      data[,i] =  -quantile(alv[,i])[3]*data[,i]
    }
  ...
  if(unique(data$VG)=="original&alpha"){
    ...
    colnames(alv)=NPIname
    for (i in NPIname){
      data1[,i] =  -quantile(alv[,i])[2]*data1[,i]
    }
  }else if(unique(data$VG)=="delta"){
    alv = al[,(length(NPIname)+1):(2*length(NPIname))] 
    colnames(alv)=NPIname
  ...
  if(unique(data$VG)=="original&alpha"){
    ...
    colnames(alv)=NPIname
    for (i in NPIname){
      data2[,i] =  -quantile(alv[,i])[4]*data2[,i]
    }
  }else if(unique(data$VG)=="delta"){
    alv = al[,(length(NPIname)+1):(2*length(NPIname))] 
    colnames(alv)=NPIname
