...
###########################################################################
out.ID = read.csv(paste0(dir,"/alldata_group_Qstatistics.csv"), stringsAsFactors = F)
#casesISO = read.csv(paste0(dir,"/NPI&cases_smooth.csv"),stringsAsFactors = F)
casesISO = all
...
pdf(paste0(outdir,"/Rtplot_city.pdf"))
...
for (i in 1:nrow(out.ID)){
  ...
  if(pand$variant == "omicron"){
    res_parametric_si <- estimate_R(cases$I,
